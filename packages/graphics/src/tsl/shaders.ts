 ...
 */
import { Fn, vec3, float, normalize, time, cos, sin } from 'three/tsl';

/** Procedural noise displacement for graph node surfaces. */
export const noiseDisplacement = Fn(([position]: [any]) => {
  const n = cos(position.x.mul(time)).add(sin(position.y.mul(time))).add(cos(position.z.mul(time.mul(0.5))));
