...
export function createStatusRing(buttonEl: HTMLElement, opts: StatusRingOptions = {}): StatusRing {
  ...
  const gradId = `substrate-sr-grad-${++_gradSeq}`;

  const zone = document.createElement('div');
  zone.className = 'substrate-sr-zone';
  zone.style.display = 'none';

  const wrap = document.createElement('div');
  wrap.className = 'substrate-sr-wrap';

  const svg = document.createElementNS(SVG_NS, 'svg') as SVGSVGElement;
  svg.setAttribute('class', 'substrate-sr-svg');
  ...
  track.setAttribute('stroke', 'var(--substrate-bg-muted)');
  track.setAttribute('stroke-width', '2.5');
  svg.appendChild(track);

  const turn = document.createElementNS(SVG_NS, 'g');
  turn.setAttribute('class', 'substrate-sr-turn');

  const fill = document.createElementNS(SVG_NS, 'circle');
  fill.setAttribute('class', 'substrate-sr-fill');
  ...
  center.className = 'substrate-sr-center';
  center.textContent = '—';
  ...
  const showIcon = (kind: 'check' | 'cross', color: string) => {
    ...
    iconSvg.setAttribute('class', 'substrate-sr-icon');
    ...
    if (kind === 'check') {
      ...
      p.style.animation = 'substrate-sr-draw 0.45s cubic-bezier(0.65,0,0.35,1) forwards';
      ...
      l1.style.animation = 'substrate-sr-draw 0.22s cubic-bezier(0.65,0,0.35,1) forwards';
      ...
      l2.style.animation = 'substrate-sr-draw 0.22s cubic-bezier(0.65,0,0.35,1) 0.14s forwards';
  ...
  const showText = (text: string) => {
    ...
    span.className = 'substrate-sr-pct';
  ...
  const hide = () => {
    ...
    fill.classList.remove('substrate-sr-dashing');
    turn.classList.remove('substrate-sr-spinning');
    resetStroke();
    setOffset(0);
    showText('—');
  };
  ...
  const setIndeterminate = () => {
    clearRevert();
    resetStroke();
    fill.classList.add('substrate-sr-dashing');
    turn.classList.add('substrate-sr-spinning');
    center.replaceChildren();
    center.style.color = '';
  };
