...
import { insertArticleSchema, insertExperimentSchema, selectArticleSchema } from '../schemas';
...
describe('insertExperimentSchema', () => {
  ...
  it('accepts any string as subsystem', () => {
    const result = insertExperimentSchema.safeParse({ ...validInput, subsystem: 'custom' });
    expect(result.success).toBe(true);
  });
