    ...
    async function detect() {
      ...
      try {
        ...
        const limits = Object.fromEntries(
          [...adapter.limits.entries()].sort(([a], [b]) => a.localeCompare(b)),
        );
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '1.5rem' }}>
        <span className="lab-badge" style={{ background: tierColors[info.tier], color: '#000', fontSize: 13 }}>
      ...
      {info.tier === 'static' && (
        <div className="lab-card" style={{ marginBottom: '1rem' }}>
          ...
          <p style={{ marginTop: '0.5rem', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
            Your browser does not support WebGPU. The platform will use
            WebGL2, Canvas 2D, or static SVG fallback depending on context.
      ...
      {info.tier === 'webgpu' && (
        <>
          ...
          <div className="lab-card" style={{ marginBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.1rem', marginBottom: '0.75rem' }}>Supported Features ({info.features.length})</h2>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem' }}>
              {info.features.map((f) => (
                <span key={f} className="lab-badge" style={{ background: 'var(--accent-dim)', color: 'var(--accent)', fontSize: 10 }}>
                  {f}
                </span>
              ))}
            </div>
          ...
          <div className="lab-card">
            <h2 style={{ fontSize: '1.1rem', marginBottom: '0.75rem' }}>Limits ({Object.keys(info.limits).length})</h2>
            <details>
              <summary style={{ cursor: 'pointer', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Show all limits</summary>
              <dl style={{ marginTop: '0.75rem', display: 'grid', gridTemplateColumns: 'max-content 1fr', gap: '0.25rem 1rem', fontSize: '0.75rem' }}>
                {Object.entries(info.limits).map(([k, v]) => (
                  <div key={k} style={{ display: 'contents' }}>
                    <dt style={{ color: 'var(--text-tertiary)' }}>{k}</dt>
                    <dd style={{ fontFamily: 'var(--font-geist-mono), monospace' }}>{v.toLocaleString()}</dd>
                  </div>
                ))}
              </dl>
            </details>
